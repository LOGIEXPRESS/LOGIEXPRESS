...
router.post('/review/carrier', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        let reviewUser = yield Review_1.Review.findAll({ where: { travelId: idTravel } }); //deberia tomar travelId 
        if (!reviewUser.length) {
            return res.status(400).json({ mensaje: 'Rewiew not found' });
        }
        else {
            let upDating = yield reviewUser[0].update({ User_raiting, User_comment });
