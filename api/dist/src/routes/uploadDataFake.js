...
router.get('/uploadDataFake', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const user_regs = yield User_Reg_1.User_Reg.bulkCreate(userFake);
    let carrier = yield Carrier_1.Carrier.bulkCreate(carrierFake);
    let track = yield Vehicle_1.Vehicle.bulkCreate(vehicleFake);
    //const userTrue = await User_Reg.findAll({where:{role:true},attributes:[['id','UserRegId']]})  
    const userTrues = yield User_1.User.bulkCreate(idUser);
